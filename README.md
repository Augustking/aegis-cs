...
**客户咨询页**：AI 澄清追问（不编造到账时间）→ 转人工后人工回复自动到达，AI / 人工署名清晰：

![客户咨询页](docs/screenshots/customer-chat.png)

**坐席三栏处理台**：队列（质检分/状态）+ 完整对话（含人工回复闭环）+ 质检维度（相关性/解答度/可信度）：

![坐席处理台](docs/screenshots/agent-inbox.png)
...
| 构造坏例集拦截率（阈值 6） | **100%**（10/10） | 接地质检上线后全部拦截；拦截手段为可信度一票否决而非仅总分 |
| 业务问题转人工比例（阈值 6） | **20%**（5/25） | 口径说明：未经人工逐条核验，不称"误拦率"；分数分布见扫描表 |
| 单轮延迟 | p50 **19.8s** / p95 33.0s | 每轮 3 次 LLM 调用（分类 + 应答 + 质检），串行；模型分层路由后大幅下降 |

**阈值扫描（拦截率 - 误拦率权衡）**：

| 阈值 | 3 | 4 | 5 | 6 | 7 | 8 | 9 |
|---|---|---|---|---|---|---|---|
| 坏例拦截率 TPR | 30% | 80% | 90% | 100% | 100% | 100% | 100% |
| 业务问题转人工比例 FPR | 8% | 16% | 20% | 20% | 32% | 36% | 40% |

![threshold](docs/threshold_curve.png)

**定标结论（两次扫描的完整故事）**：judge 模型从 Qwen2.5-7B 升级到 Qwen3-8B 后分数分布显著变化，默认阈值经重新扫描从 8 回调至 6——同档拦截率 60%→100%、转人工比例 36%→20%，阈值 6 严格优于 8。核心教训：**分数分布随模型版本变化，阈值必须随每次模型升级重新标定**，不能一次性拍脑袋定死。
