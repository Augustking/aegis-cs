...
自建评测集（`evals/eval_set.json`）：30 题意图分类集（六桶 × 5，覆盖产品/技术/账单/投诉/一般咨询/护栏）+ 10 条构造坏例集（编造承诺、答非所问、荒谬建议等六类缺陷）+ 25 题真实链路运行。评测脚本 `evals/run_eval.py` 支持断点续跑与并发，一条命令复现全部数字：
...
| 指标 | 数值 | 说明 |
|---|---|---|
| 意图分类准确率 | **86.7%**（26/30） | 四个业务桶 100%；错误集中于闲聊→general 的语义模糊带 |
| 坏例拦截率（阈值 6） | **90%**（9/10） | 10 条构造坏例中 9 条被判低于阈值转人工 |
| 正常回复误拦率（阈值 6） | **16%**（4/25） | 全部发生在 7 分（贴近阈值的边缘样本） |
| 单轮延迟 | p50 82s / p95 118s | 每轮 3 次 LLM 调用（分类 + 应答 + 质检），串行 |

**阈值扫描（拦截率 - 误拦率权衡）**：

| 阈值 | 3 | 4 | 5 | 6 | 7 | 8 | 9 |
|---|---|---|---|---|---|---|---|
| 坏例拦截率 TPR | 0% | 90% | 90% | 90% | 90% | 100% | 100% |
| 正常误拦率 FPR | 0% | 12% | 12% | 16% | 16% | 92% | 92% |

![threshold](docs/threshold_curve.png)

扫描结论：**4-7 是可用的均衡区间**（拦截率稳定 90%、误拦 ≤16%），阈值 8 起 judge 分数分布整体左移导致大面积误拦，印证阈值不能拍脑袋定、必须用评测集标定。评测中的 4 条误拦样本全部是 7 分边缘案例，可作为 judge 提示词迭代的坏例输入。
